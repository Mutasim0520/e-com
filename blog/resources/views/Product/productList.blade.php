                        ...
                        <table class="table table-hover" id="myTable">
                            ...
                            @foreach($Products as $item)
                                <tr>
                                    ...
                                    @else
                                        <td>{{ $item->code }}</td>
                                        <td><a href="/employee/indivisualProduct/{{ encrypt($item->product_id) }}">{{ $item->title }}</a></td>
                                        @if(intval($item->quantity)>0)
                                            <td>{{ $item->quantity }}</td>
                                        @elseif(intval($item->quantity)==0) <td>No Product Available</td>
                                        @else<td>Shortage: {{ intval($item->quantity)*-1 }}</td>
                                        ...
                                        <td>{{ $item->price }}</td>
                                        <td>{{ $item->discout }}%</td>
                                        <td><a class="btn btn-large btn-primary" href="/employee/updateProduct/{{encrypt($item->product_id)}}">Update</a></td>
                                    @endif
                                </tr>
                                <?php $i = $i+1; ?>
                            @endforeach
